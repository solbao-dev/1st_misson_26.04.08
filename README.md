...
**`Issue 2` : Docker 데몬 연결 실패 (Cannot connect to the Docker daemon)**
...
**`Issue 3` : GitHub CLI 인증 실패 (Support for password authentication was removed)**
...
* **해결 및 대안:** GitHub 웹의 `Developer settings`에서 `repo` 권한이 부여된 터미널 전용 토큰(Token)을 발급받아, 비밀번호 입력란에 토큰을 대신 입력하여 인증진행
---
